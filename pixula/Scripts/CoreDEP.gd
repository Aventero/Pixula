...
enum MATERIAL {PROCESSED_BIT = 0, AIR = 37, STONE = 41, SAND = 22, WATER = 2}

var pixel_dictionary : Dictionary

var pixel_array_2d = []
var pixel_array_2d_changed = []
...
func _process(_delta: float) -> void:
	...
	if Input.is_action_pressed("SPAWN_WATER"):
		Input.mouse_mode = Input.MOUSE_MODE_HIDDEN
		set_particle(get_mouse_tile_pos().x, get_mouse_tile_pos().y, get_random_water())
...
func setup_environment() -> void:
	setup_array(pixel_array_2d, MATERIAL.AIR)
	setup_array(pixel_array_2d_changed, MATERIAL.PROCESSED_BIT)
	print("Amount of pixels: ", width * height)

func setup_array(array_to_set_up : Array, mat : MATERIAL) -> void:
...
func transfer_to_tilemap() -> void:
	for y in range(0, height):
		for x in range(0, width):
			map.set_cell(Vector2i(x, y), 1, Vector2i(pixel_array_2d[y][x], 0), 0)
...
func water_mechanic(x : int, y : int) -> void:
	if not sand_mechanic(x, y):
		if pixel_array_2d[y][x - 1] == MATERIAL.AIR && randi_range(0, 1) == 0:
			swap_particle(x, y, x - 1, y)
			return
		elif pixel_array_2d[y][x + 1] == MATERIAL.AIR && randi_range(0, 1) == 0:
			swap_particle(x, y, x + 1, y)
			return

func swap_particle(posAx : int, posAy : int, posBx : int, posBy : int) -> void:
	var tmp = pixel_array_2d[posBy][posBx]
	pixel_array_2d[posBy][posBx] = pixel_array_2d[posAy][posAx]
	pixel_array_2d[posAy][posAx] = tmp
